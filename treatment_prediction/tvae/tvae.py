...
class SinkhornDistance(nn.Module):
    ...
    """
    def __init__(self, eps, max_iter, device='cpu', reduction='none'):
...
class DisentangledVAE:
    ...
    def _trainer(self, train_data, targets,mask):
        ...
        # train_data,test_data,train_label,eval_data, ite_train, ite_eval,ite_test
...
def init_tvae_model(input_dimension=25, batch_size=32):
    VAE = DisentangledVAE(n_epochs=100, number_of_labels=3,input_dimension =input_dimension,
                      latent_dimension = 10, 
                      hidden_layer_width=500,device=torch.device('cuda'))
    
    VAE.batch_size=batch_size
    VAE.optimizer = optim.Adam(
        list(VAE.encoder.parameters()) + list(VAE.decoder.parameters()), lr=1e-5
        # ,weight_decay=1e-5
    )
    #prediction loss
    VAE.pred_weight = [1,1,1]
    # KL loss
    VAE.KL_weight=1
    #TC loss
    VAE.beta = .1
...
def train_vae_model(model, X, T, Y):
    labels = torch.ones([len(Y), 3])
    labels[:, 0] = T
    labels[T==0, 1] = Y[T==0].view(-1)
    labels[T==1, 2] = Y[T==1].view(-1)
    
    for epoch in trange(model.n_epochs, desc='epochs',leave=False):
            epoch_train_loss=[]
            train_set = torch.utils.data.TensorDataset(X, torch.ones_like(X), labels)
            train_loader = DataLoader(train_set, shuffle=True,
                                      num_workers=1, drop_last=True, 
            ...
            for i, batch_data in enumerate(train_loader,0):
                ...
                (loss, pred_loss, 
                 recon_loss,original_KL,
                 tc_loss, mmd_loss) = model._trainer(data.to(model.device),
...
def eval_vae_outcome(model, X, T):
    if type(X) is np.ndarray:
        X = torch.from_numpy(X).float()
    if type(T) is np.ndarray:
        T = torch.from_numpy(T).float()
    pred_y = torch.zeros_like(T).to(model.device).type(torch.float)
    
    with torch.no_grad():
        epoch_pred = model.encoder(torch.Tensor(X).to(model.device))        
    
    pred_y[T==0] = epoch_pred[T==0,1]
    pred_y[T==1] = epoch_pred[T==1,2]
    
    return pred_y.cpu().detach().numpy()

def eval_vae_ite(model, X, T):
    pred_y = torch.zeros_like(T)  
    
    with torch.no_grad():
        epoch_pred = model.encoder(torch.Tensor(X).to(model.device))        
    
    ite = epoch_pred[:,2] - epoch_pred[:,1]
    
    return ite.cpu().detach().numpy()