...
out_root_folder="/data6/wuyinjun/causal_tabular/tcga/"
...
#	                mkdir -p ${out_folder}
        	        ...
        	        mkdir -p ${out_folder}
	                out_folder=${out_folder}/${method}_reg/
			mkdir -p ${out_folder}
			...
			cmd="python train_tabular.py --gpu_db  --epochs 500 --num_treatments 3 --dataset_name tcga --batch_size 128 --lr 1e-4 --topk_act ${topk_act} --program_max_len ${program_max_len} --model_config configs/configs_tcga.yaml --data_folder /data6/wuyinjun/causal_tabular/  --method ${method} --seed ${seed}  --log_folder $out_folder --backbone TransTEE --cached_backbone /data6/wuyinjun/causal_tabular/tcga//seed_${seed}/TransTEE/logs/bestmod.pt $fix_model --regression_ratio  ${regress_ratio}"
