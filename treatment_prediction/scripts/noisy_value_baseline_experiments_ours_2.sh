...
do
	...
	do
		...
		do
			...
			mkdir -p ${out_folder}
			out_folder=${out_folder}/logs_2/
			mkdir -p $out_folder
			
			echo "method::$method, noise_ratio::${ratio}, random_seed:${seed}"
			echo "output folder:: ${out_folder}"
			init_cmd="python train_tabular_rl.py --epochs 100 --num_treatments 2 --dataset_name ihdp --batch_size 32 --lr 1e-3 --topk_act 1 --program_max_len 6 --model_config configs/configs_ihdp.yaml --data_folder /data2/wuyinjun/causal_tabular/ --noise_mag ${ratio}  --method ${method} --seed $seed --backbone transtee  --dataset_id"
