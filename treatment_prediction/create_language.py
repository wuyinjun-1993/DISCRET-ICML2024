...
class Language:
    def __init__(self, data, id_attr, outcome_attr, count_outcome_attr, treatment_attr, does_attr, text_attr, precomputed, lang, num_feats=None):
        self.lang = lang
        self.syntax = lang.LANG_SYNTAX
        self.dataset = data
        self.precomputed = precomputed
        if num_feats is None:
            num_feats = [col for col in self.dataset.columns if col not in lang.CAT_FEATS and col not in lang.DROP_FEATS and not col == id_attr and not col == outcome_attr and not col == treatment_attr and not col == text_attr and not col == count_outcome_attr and not col == does_attr] 
    ...
    def evaluate_atom_ls_ls_on_dataset_full_multi_backup(self, expr_ls, full_data_ls, col_name_col, op_name_col, pred_v_col, other_keys=None):
    ...
    def evaluate_atom_ls_ls_on_dataset_full_multi(self, expr_ls, full_data_ls, col_id_col, op_name_col, pred_v_col, other_keys=None):

        existing_data_ls = []
        transformed_expr_ls = []
        pred_v_ls = expr_ls[pred_v_col]
        col_id_ls = expr_ls[other_keys[0]]
        further_selection_mask = expr_ls[other_keys[1]]
        op_ls = expr_ls[op_name_col]
        
        for idx in range(len(pred_v_ls)):
            target_const = pred_v_ls[idx]
            data_ls = full_data_ls[idx]
            sub_col_id_ls = col_id_ls[idx]
            sub_op_ls = op_ls[idx]
            curr_further_selection_mask = further_selection_mask[idx]
            curr_existing_data_ls = []
            curr_expr_ls = []
            # for data in data_ls:
            '''previous version'''
            for sub_idx in range(len(sub_op_ls)):                  
                existing_data = data_ls[sub_idx]
                if not curr_further_selection_mask:
                    curr_existing_data_ls.append(existing_data)
                # for expr in expr_ls:
                    # assert "formula" in expr
                    # if expr["formula"] == "end":
    ...
    def atom_to_str_ls_full(self, X_pd_ls, expr_ls, col_name_key, op_name_key, col_v_key, feat_range_mappings, cat_id_unique_vals_mappings, other_keys=None):
        ...
        col_id_arr = expr_ls[col_id_key].tolist()
        further_selection_mask = expr_ls[other_keys[1]]
        # for expr in expr_ls:
        # for const in const_arr:
                ...
                for sub_idx in range(len(col_name_arr[idx])):
                    ...
                    if not col_name_arr[idx][sub_idx] in self.lang.CAT_FEATS:
                        if feat_range_mappings is not None:
                            ...
                            real_val = const_arr[idx][sub_idx]
